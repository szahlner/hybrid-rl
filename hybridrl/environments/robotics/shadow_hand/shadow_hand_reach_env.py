...
MODEL_PATH = os.path.join(os.path.dirname(__file__), '..', 'assets', 'urdf', 'shadow_hand', 'shadow_hand_right.urdf')
SPHERE_GREEN_PATH = os.path.join(os.path.dirname(__file__), '..', 'assets', 'urdf', 'common', 'sphere_green.urdf')
SPHERE_BLUE_PATH = os.path.join(os.path.dirname(__file__), '..', 'assets', 'urdf', 'common', 'sphere_blue.urdf')
SPHERE_RED_PATH = os.path.join(os.path.dirname(__file__), '..', 'assets', 'urdf', 'common', 'sphere_red.urdf')
SPHERE_YELLOW_PATH = os.path.join(os.path.dirname(__file__), '..', 'assets', 'urdf', 'common', 'sphere_yellow.urdf')
SPHERE_CYAN_PATH = os.path.join(os.path.dirname(__file__), '..', 'assets', 'urdf', 'common', 'sphere_cyan.urdf')
...
class ShadowHandReachGoalEnvAllRandom(PyBulletGoalEnv, utils.EzPickle):
    """Shadow Hand Reach goal environment all random.
    ...
    def __init__(self,
        ...
        super(ShadowHandReachGoalEnvAllRandom, self).__init__(model_path=model_path,
                                                              initial_pos=initial_pos,
                                                              sim_time_step=sim_time_step,
                                                              sim_frames_skip=sim_frames_skip,
                                                              sim_n_sub_steps=sim_n_sub_steps,
    ...
    def sample_goal(self):
        """Randomly chose goal."""
        goals = np.concatenate([np.array(GOAL_POSITIOS).copy(),
                                np.expand_dims(self.get_current_position(), axis=0)], axis=0)
        choice = self.np_random.choice(goals.shape[0], 5)
        goal = np.empty(shape=[5, 3])
        for n in range(goal.shape[0]):
            goal[n, :] = goals[choice[n], n, :]
