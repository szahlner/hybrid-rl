...
def main():  # noqa: C901
    ...
    # Save buffer
    n_state = model.observation_space.shape[0]
    n_action = model.action_space.shape[0]
    ...
    if model.device.type == "cpu":
        ...
        use_cuda = True
    buffer = ReplayBuffer(
        n_state=n_state, n_action=n_action, size=args.n_timesteps, use_cuda=use_cuda
    )
    try:
        for _ in range(args.n_timesteps):
            ...
            # Store in buffer
            buffer.add(
                state=obs, action=action, reward=reward, mask=done, next_state=next_obs
            )
    ...
    # Save buffer
    buffer.save(os.path.join(log_path, 'recorded_buffer.npz'))
