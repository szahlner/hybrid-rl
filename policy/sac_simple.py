...
class SAC:
    ...
    def learn(self):
        ...
        for ts in range(int(self.args.max_timesteps)):
            ...
            if self.args.model_based:
                ...
                # Model based section
                if ts >= self.args.start_timesteps and ts > self.args.model_training_freq:
    ...
    def _unreal_update_network(self):
        ...
        with torch.no_grad():
            ...
            if pi.size(1) == 1:
                transitions["actions"] = pi.cpu().numpy()
            else:
                transitions["actions"] = pi.cpu().numpy().squeeze()

        diff = np.empty((self.args.batch_size, self.env_params["obs"] + self.env_params["reward"]))
        confidence = np.empty((self.args.batch_size, self.env_params["obs"] + self.env_params["reward"]))

        for n in range(len(self.model_chunks) - 1):
            diff_, confidence_ = self.world_models[n].predict(
                inputs=np.concatenate([transitions["obs"], transitions["actions"]], axis=-1)
            )
            diff[:, self.model_chunks[n]:self.model_chunks[n + 1]] = diff_
            confidence[:, self.model_chunks[n]:self.model_chunks[n + 1]] = confidence_

        transitions["obs_next"] = transitions["obs"] + diff[:, :self.env_params["obs"]]
        transitions["r"] = diff[:, self.env_params["obs"]:]
