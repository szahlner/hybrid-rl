...
class Normalizer:
    ...
    def sync(self, local_sum: np.ndarray, local_sumsq: np.ndarray, local_count: np.ndarray):
    ...
    def _mpi_average(self, x: np.ndarray):
        buf = np.zeros_like(x)
    ...
    def normalize(self, v: np.ndarray, clip_range: Optional[float] = None):
