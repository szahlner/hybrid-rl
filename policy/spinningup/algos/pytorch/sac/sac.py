...
import gym
import time
import policy.spinningup.algos.pytorch.sac.core as core
from policy.spinningup.utils.logx import EpochLogger
...
def sac(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=123,
        ...
        update_after=None, update_every=1, num_test_episodes=10, max_ep_len=1000,
        logger_kwargs=dict(), save_freq=1,
        auto_alpha=True, target_entropy=-1, args=None):
    """
    ...
    # Set up model based stuff
    if args.model_based:
        env_params = get_env_params(env)

        model_dim_chunk = args.model_dim_chunk
        output_dim = env_params["obs"] + env_params["reward"]
        model_chunks = [model_dim_chunk for _ in range(output_dim // model_dim_chunk)]
        if output_dim % model_dim_chunk != 0:
            model_chunks.append(output_dim % model_dim_chunk)
        ...
        model_chunks.insert(0, 0)
        model_chunks = np.cumsum(model_chunks).tolist()

        # Buffers
        simple_world_model_replay_buffer = MyReplayBuffer(env_params, replay_size)

        # Create the replay buffer
        world_model_params = deepcopy(env_params)
        world_model_params["max_timesteps"] = args.model_max_rollout_timesteps  # change max timesteps to rollout length
        world_model_buffer = MyReplayBuffer(world_model_params, replay_size)
    ...
    def update(data):
        ...
        loss_pi, pi_info, logp_pi = compute_loss_pi(data)
        loss_pi.backward()
        pi_optimizer.step()
    ...
    for t in range(total_steps):
        ...
        if t >= start_steps and t % steps_per_epoch == 0:
